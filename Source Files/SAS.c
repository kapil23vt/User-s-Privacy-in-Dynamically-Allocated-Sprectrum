...
//    gmp_printf("\n%Zd", Z);                
    ...
    for(l = 0; l < L; l++){               
        for(h = 0; h < H; h++){
            for(f = 0; f < F; f++){
//                printf("\nN before encryption is");
//                gmp_printf("\n%Zd", *(N_enc+offset(l,h,f)));
        ...
        for(l = 0; l < L; l++){               
            for(h = 0; h < H; h++){
                for(f = 0; f < F; f++){
                    ...
                    //printf("\n\nT_i_enc is");
                    //gmp_printf("\n\n%Zd", *(T_i_enc+offset(l,h,f)));
                    //Add homomorphically if not equal to zero
                    mpz_t temp_zero3; //creating temp_zero variable
                    mpz_inits(temp_zero3, NULL);
                    int temp_result3 = mpz_cmp (*(T_i_enc+offset(l,h,f)),temp_zero3);
                    if(temp_result3 != 0)
                    {
    ...
    for(l_i = 0; l_i < L; l_i++){
        for(h_i = 0; h_i < H; h_i++){
            for(f_i = 0; f_i < F; f_i++){
                ...
                //gmp_printf("\nF is %Zd", F_enc+offset(l_i,h_i,f_i));
                //gmp_printf("\nR_enc is %Zd", *(R_enc+offset(l_i,h_i,f_i)));
                //gmp_printf("\nF_enc before is %Zd", *(F_enc+offset(l_i,h_i,f_i)));
                ...
                for(l_s = 0; l_s < L; l_s++){
                    for(h_s = 0; h_s < H; h_s++){
                        for(f_s = 0; f_s < F; f_s++){
                            ...
                            mpz_t temp_zero; //creating temp_zero variable
                            mpz_inits(temp_zero, NULL);
                            // temp_zero to be used for encryption of non-zero values
                            //temp_holder = l;
                            int temp_result = mpz_cmp (*(R_enc+ offset(l_s,h_s,f_s)),temp_zero);
                            //printf("\n\ntemp_result is %d", temp_result);
                            //gmp_printf("\nHere R_enc is %Zd", *(R_enc+offset(l_s,h_s,f_s)));
                            //gmp_printf("\nI is %Zd", *(I+ l_i*L*H*H*F*F + h_i*L*H*F*F+ f_i*L*H*F + l_s*H*F +h_s*F + f_s));
                            if(temp_result)
                            {
                                //gmp_printf("\n\n\n\n\nHere R_enc is %Zd", *(R_enc+offset(l_s,h_s,f_s)));
                            ...
                            //gmp_printf("\nNow F_enc is %Zd", *(F_enc+offset(l_s,h_s,f_s)));
                            //gmp_printf("\ntemp is %Zd", temp);
                            //paillier_homomorphic_add(*(F_enc+offset(l_i,h_i,f_i)), *(F_enc+offset(l_i,h_i,f_i)), temp, pub);
                        }
                    }
                }
                //gmp_printf("\nNow F_enc is %Zd", *(F_enc+offset(l_i,h_i,f_i)));
                paillier_homomorphic_sub(*(G_enc+offset(l_i,h_i,f_i)), *(N_enc+offset(l_i,h_i,f_i)), *(F_enc+offset(l_i,h_i,f_i)), pub);
                //gmp_printf("\nG is %Zd", G_enc+offset(l_i,h_i,f_i));
            }
        }
    }
    return G_enc;
