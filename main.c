...
//    benchmark(); //benchmark for Paillier cryptosystem
    ...
    if (1){
        ...
        for(l = 0; l < L; l++){               
            for(h = 0; h < H; h++){
                for(f = 0; f < F; f++){
                    //gmp_printf("xxxx %Zd\n", *(N_enc + offset(l,h,f)));
                    mpz_t temp_zero2; //creating temp_zero variable
                    mpz_inits(temp_zero2, NULL);
                    int temp_result2 = mpz_cmp (*(N_enc+offset(l,h,f)),temp_zero2);
                    printf("\nlocation is %d", offset(l,h,f));
                    if(temp_result2 != 0)
                    {
                        //gmp_printf("\nValue %Zd\n", *(N_enc + offset(l,h,f)));
                        paillier_decrypt(temp, *(N_enc + offset(l,h,f)), &priv);
                        gmp_printf("\nDecrypted value is %Zd\n", temp);
                    }
                    else
                    {
                        gmp_printf("\nValue %Zd\n", *(N_enc + offset(l,h,f)));
    ...
    printf("\nTime taken to execute is %f ", time_taken);
    return 0;
}
